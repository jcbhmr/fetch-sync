...
const BodySyncMixin = {
  prototype: {
    /**
     * `.body` is now a `Uint8Array` instead. Normally, `.body` is a
     * `ReadableStream<Uint8Array>`, but since we are in synchronous land, that
     * won't cut it. The next-closest thing is either an `ArrayBuffer` or a raw
     * complete `Uint8Array`. In this case, we have chosen the `Uint8Array`
     * option since it fits with the `Uint8Array` from the `ReadableStream`.
     */
    ...
    /**
     * Assumes the body is encoded as UTF-8. Parses the body as JSON and
     * consumes it.
     *
